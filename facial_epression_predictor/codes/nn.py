...
from utils import load_data, load, save, display_plot, save_images
import numpy as np
...
def affine_backward(grad_y, x, w):
    ...
    :param w: Weights
    :return: A tuple of (grad_x, grad_w, grad_b)
        WHERE
...
def train(model, forward, backward, update, eta, num_epochs, batch_size):
    ...
    # error = 1 - accuracy
    train_error_list = [(t[0], 1 - t[1]) for t in train_acc_list]
    valid_error_list = [(t[0], 1 - t[1]) for t in valid_acc_list]
    display_plot(train_error_list, valid_error_list, "Error rate", number=1)
    ...
    print("Cross Entropy: Train %.5f Validation %.5f Test %.5f" %
          (train_ce, valid_ce, test_ce))
    print("Accuracy: Train {:.5f} Validation {:.5f} Test {:.5f}".format(
        train_acc, valid_acc, test_acc))
    print("Error rate: Train {:.5f} Validation {:.5f} Test {:.5f}".format(
        1 - train_acc, 1 - valid_acc, 1 - test_acc))
...
def plot_five_uncertain_examples(inputs, target, model, forward, threshold, filename):
    """
    Find 5 uncertain examples (predicted top score by model is below threshold)
    and save them to a plot.
    """
    examples = np.zeros((5, inputs.shape[1]))
    j = 0
    i = 0
    correct = []
    while j < 5 and i < inputs.shape[0]:
        x = inputs[i]
        t = target[i]
        y = forward(model, x)["y"]
        prediction = np.exp(y) / np.exp(y).sum()
        if max(prediction) < threshold:
            examples[j] = x
            j += 1
            # whether the prediction is correct
            correct.append(np.where(prediction == max(prediction)) == np.where(t == 1))
        i += 1
    if j == 5:
        save_images(examples, filename, 5)
        print(correct)
        print("The accuracy is {}.".format(sum(correct) / len(correct)))
    else:
        print("less than 5 examples found, please choose another threshold value.")


def plot_examples_from_category(inputs, target, model, forward, category, filename):
    """
    Find 10 examples from corresponding category
    and save them to a plot.
    """
    examples = np.zeros((10, inputs.shape[1]))
    j = 0
    i = 0
    correct = []
    while j < 10 and i < inputs.shape[0]:
        x = inputs[i]
        t = target[i]
        y = forward(model, x)["y"]
        prediction = np.exp(y) / np.exp(y).sum()
        if np.where(t == 1)[0] == category:
            examples[j] = x
            j += 1
            # whether the prediction is correct
            correct.append(np.where(prediction == max(prediction)) == np.where(t == 1))
        i += 1
    if j == 10:
        save_images(examples, filename, 10)
        print(correct)
        print("The accuracy is {}.".format(sum(correct) / len(correct)))
    else:
        print("less than 10 examples found.")
...
def main():
    ...
    stats = load(stats_file_name)

    # plot five uncertain examples from the validation set
    # plot_five_uncertain_examples(inputs_valid, target_valid, model, nn_forward, 0.415, "examples.png")

    # plot 10 examples from a given category
    for category in range(0, 7):
        plot_examples_from_category(inputs_valid, target_valid, model, nn_forward, category,
                                    "examples_from_category_{}.png".format(category))
